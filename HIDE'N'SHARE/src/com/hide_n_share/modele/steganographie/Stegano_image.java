...
public class Stegano_image extends Steganographie{
	...
	public void dissimulerDonnee(String cheminEnveloppeModifier, boolean compresserLettre, String mdp){
		if ((this.verificationCompabilite(compresserLettre, mdp))) {
			try{
				Bitmap enveloppe = BitmapFactory.decodeFile(this.enveloppe.getEnveloppe());				
	...
	public boolean verificationCompabilite(boolean compresser, String mdp){		
		...
		if (tmpString.length ==2 ){
			...
			if(formatConvertissable.contains(tmpString[1])){
				System.out.println("le format de l'image n'est pas compatible, il sera donc converti en format bmp, (penser a changer l'extension de l'image modifier !)");
				File enveloppeTmp = new File("fichierTemporaire/imageTemporaireConverti.bmp");
				
				
				try{
					Image img = ImageIO.read(new File(enveloppe.getEnveloppe()));
					BufferedImage envTest = (BufferedImage)img;
					ImageIO.write(envTest,"bmp", enveloppeTmp);
					enveloppe.setEnveloppe("fichierTemporaire/imageTemporaireConverti.bmp");
					
					while(!enveloppeTmp.exists()){}
					
					img = ImageIO.read(new File(enveloppe.getEnveloppe()));
					envTest = (BufferedImage)img;
